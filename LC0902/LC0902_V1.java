...
public class LC0902_V1 {
    public int atMostNGivenDigitSet(String[] digits, int n) {
        ...
        int a = (BinarySearch(StrtoInt,Ndigits[0]))-1;
//        a = (a<0) ? 0 : a;
        if (a>0){
            sum  += (a * Math.pow(lenSI, Ndigits.length-1));//小于最高位
            for (int i=1;i<Ndigits.length;i++){
                ind = BinarySearch(StrtoInt,Ndigits[i]);
                last*=ind;
            }
            return sum+last;
        }
        else if (a == 0){//a==0有两种情况，(Ndigits[0] == StrtoInt[0])和(Ndigits[0] > StrtoInt[0])
//            a += 1;
//            sum  += (a * Math.pow(lenSI, Ndigits.length-1));//小于最高位
//            if ((Ndigits[0] == StrtoInt[0]) || (Ndigits.length == 1)) return sum;
//            else {
//                for (int i=1;i<Ndigits.length;i++){
//                    ind = BinarySearch(StrtoInt,Ndigits[i]);
//                    last*=ind;
...
//        System.out.println(Arrays.toString(StrtoInt));
//        Ndigits = culLen(n);
    }
